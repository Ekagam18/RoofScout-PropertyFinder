...
function UserDashboard() {
    ...
    useEffect(() => {
        async function fetchTourRequests() {
            // Only proceed if properties data is available or loading has finished.
            if (!properties.length && !loadingProperties && userId) {
                // If loading is false and properties is empty, we still proceed to fetch, 
                // but the .in() filter will likely return empty unless the DB has inconsistencies.
                // We mainly rely on the `properties` state changing.
            }
            ...
            // Fetch requests that are directly linked to this user's listed properties
            // NOTE: Using the column names from the previous data modeling step: requested_date, requested_time, requester_name
            const { data, error } = await supabase
                .from("tour_requests")
                .select(`
                    id, 
            ...
            if (error) {
                ...
                // Ensure 'pending' status is handled case-insensitively if needed, but 'pending' should match 'Pending' or 'pending' if the database is consistent.
                setReceivedTourRequests(data || []);
            }
            setLoadingRequests(false);
        }
    ...
    // --- BUYER VIEW (My Requests & Applications) - Using local storage 'allTourRequests' as per original code ---
    const displayAppliedProperties = () => {
        // NOTE: This section still relies on local storage ('appliedProperties', 'allTourRequests') 
        // and needs to be updated to query the 'tour_requests' table using requester_id.
        // For now, we keep the original local storage logic to avoid breaking other parts.
        const appliedProperties = getStoredJSON('appliedProperties', []);
        // Re-use the buyer's requests logic from the original implementation (still relying on localStorage)
        const allTourRequests = getStoredJSON('allTourRequests', []); 
        const myTourRequests = allTourRequests.filter(req => req.buyerName === loggedUser);
